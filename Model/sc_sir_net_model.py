...
def compute_M(data, max):
    cols = np.arange(data.size)
    return csr_matrix((cols, (data.ravel(), cols)),
                      shape=(max + 1, data.size))

def get_indices_sparse(data, max):
    M = compute_M(data, max)
...
# iteration starts #
def SC_SIR_model(init_status, actual_iteration, num_iteration, beta, tri_beta, gamma, graph, triangles, node_status=True):
    status = init_status
    nodes_status = np.zeros(shape=(num_iteration, len(graph)), dtype=np.float32)
    g = graph
    all_indices = get_indices_sparse(triangles, max = len(net.nodes()))
    while actual_iteration < num_iteration:
        ...
        for u in g.nodes:
            ...
            tmp_index = all_indices[u][0]
            triangles_u_ = triangles[tmp_index]
            triangles_u = (triangles_u_[triangles_u_ != u].reshape(-1, 2)).tolist()
            num_infected_triangles = np.sum([1 if status[x] + status[y] == 2 else 0 for x, y in triangles_u])
            ...
            if u_status == 0:
                if eventp < 1 - (1-beta)**len(infected_neighbors):
                    ...
                    if num_infected_triangles > 0:
                        if eventp < 1- (1-tri_beta)**num_infected_triangles:
                            actual_status[u] = 1
...
for i in range(sim_num):
    np.random.seed(i)
    res = SC_SIR_model(init_status=init_status, actual_iteration=0, num_iteration=days, beta=sc_sir_beta, tri_beta = sc_beta, gamma=sc_sir_gamma, graph=net, triangles = triangles)
